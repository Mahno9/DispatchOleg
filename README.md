...
`assets` — каждая таблица в свой `content/*.json`, файлы ассетов копируются в `content/assets/`.
`users` и `game_states` не трогаются никогда: **прогресс игроков заливка не сбрасывает**.
