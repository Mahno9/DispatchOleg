...
// когда игроку показывают экран победы. Чистая логика без React — её видят и мета, и
 ...
 * победы, поэтому в прогрессе и жребии его нет.
 */
...
/** Что решает показ победы: разбирается в тестах по одному полю за раз. */
export interface VictoryInput {
  games: Game[];
  results: Record<string, GameResult>;
  /** Флаг «экран победы этому игроку уже показывали» (localState). */
  victorySeen: boolean;
}
 ...
 *
 * Один показ на прохождение: ростер добит — победа, дальше держит флаг
 * `victory_seen`. Финал на это не влияет никак — он запускается кнопкой с
 * экрана победы, а брошенный на середине остаётся доступен из списка операций.
 */
export function shouldShowVictory({ games, results, victorySeen }: VictoryInput): boolean {
  return allRosterWon(games, results) && !victorySeen;
}
