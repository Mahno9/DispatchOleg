...
/** Линейная цепочка реплик: [кто, текст]. Диспетчер слева, собеседник справа. */
...
function buildDialogues() {
  ...
  return {
    ...
    'meta-marina': {
      ...
      nodes: {
        ...
        nodes: {
          n1: node(
            C.marina,
            'Диспетчер, я тебе оставила кофе. Он остыл ещё до того, как ты пришёл, но жест засчитан.',
            {
              choices: [
    ...
    'meta-timur': {
      title: 'Мета · Тимур',
      nodes: linear([
        [C.timur, 'Товарищ диспетчер, а правда, что вы однажды посадили вертолёт по рации?'],
    ...
    'pre-shift': {
      ...
      nodes: {
        start: 'n1',
        nodes: {
          n1: node(C.marina, 'Диспетчер, я досиживаю последний час, и входящие не разобраны.', {
            next: 'n2',
          }),
    ...
    'style-shift-flawless': {
      ...
      nodes: linear([
        [C.marina, 'Ни одной ошибки. Диспетчер, ты меня пугаешь.'],
    ...
    'style-metro-breaker': {
      ...
      nodes: linear([
        [C.kostya, 'Диспетчер. Мне звонили из метрополитена.'],
        [O, 'Мы вышли. Все живы.'],
    ...
    'pre-kitchen': {
      ...
      nodes: {
        start: 'n1',
        nodes: {
          n1: node(C.lida, 'Диспетчер, у меня очередь из героев и одна плита. Встань к котлу.', {
            choices: [
