...
// Ссылка из триггера меты на финал законна, но обязательной не является:
// «После смены» открывается по победам ростера, а финал запускается кнопкой с
// экрана победы — ждать его стадия не должна.
{
